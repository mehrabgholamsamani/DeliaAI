...
type BookingNotification = {
  bookingCreated(workspaceId: string, booking: {
...
const presentationSettings = {
  ...
  companyDescription:
    'A friendly neighbourhood dental practice providing preventive, restorative, and cosmetic dental care for adults and families.',
  greeting: 'Thanks for calling Northstar Dental. How can I help today?',
  assistantTone: 'warm, reassuring, concise, and professional',
  bookingInstructions: 'Offer live availability only after confirming the requested service and contact details. Always require confirmation before booking.',
  handoffInstructions: 'Offer a callback when an answer is not in the approved information or when the caller asks for a person.',
  contactDetails: 'Reception: +49 30 555 0142. Located at 18 Lindenstraße, Berlin.'
};
...
const defaultServices: ServiceInput[] = [
  {
    slug: 'dental-exam',
    name: 'Dental exam',
    description: 'A full dental check-up with a dentist and a care plan if needed.',
    priceLabel: '€75',
    durationMinutes: 60
  },
  {
    ...
    description: 'A hygienist appointment for a professional dental cleaning.',
    priceLabel: '€110',
    durationMinutes: 60
  },
  {
    slug: 'emergency-dental-visit',
    name: 'Emergency dental visit',
    description: 'An urgent assessment for dental pain, swelling, or a broken tooth.',
    priceLabel: '€95',
    durationMinutes: 45
  }
];
...
export class CrmService {
  constructor(
    ...
    @Inject(OwnerNotificationService) private readonly notifications: BookingNotification
  ) {}
  ...
  async createBooking(input: BookingInput, workspaceId = LEGACY_WORKSPACE_ID) {
    ...
    void this.notifications.bookingCreated(workspaceId, booking);
  ...
  private async ensureDefaults(workspaceId: string) {
    ...
    const placeholders = await this.prisma.service.findMany({
      where: { workspaceId, slug: { in: ['consultation', 'standard-service'] } },
      select: { id: true }
    });
    if (placeholders.length) {
      const usedServiceIds = await this.prisma.booking.findMany({
        where: { serviceId: { in: placeholders.map((item) => item.id) } },
        select: { serviceId: true },
        distinct: ['serviceId']
      });
      const unusedIds = placeholders
        .map((item) => item.id)
        .filter((id) => !usedServiceIds.some((booking) => booking.serviceId === id));
      if (unusedIds.length) await this.prisma.service.deleteMany({ where: { id: { in: unusedIds } } });
      const usedIds = placeholders.map((item) => item.id).filter((id) => !unusedIds.includes(id));
      if (usedIds.length)
        await this.prisma.service.updateMany({ where: { id: { in: usedIds } }, data: { isActive: false } });
    }
