...
export class KnowledgeService {
  ...
  async relevantFor(query: string, workspaceId = LEGACY_WORKSPACE_ID) {
    // Internal notes are for the owner dashboard, never public receptionist context.
    let articles = (await this.list(false, workspaceId)).filter((article) => article.category !== 'INTERNAL');
    ...
    return articles
      ...
      .filter((item) => item.semanticScore > 0.18 || item.lexicalScore > 0)
      .slice(0, 4)
